...
class Settings(BaseSettings):
    ...
    def validate_model_format(cls, v: str | None) -> str | None:
        if v is None:
            return None
        valid_providers = ("nvidia_nim", "open_router", "lmstudio", "ollama", "deepseek", "groq", "together", "cerebras", "sambanova", "mistral", "github", "zhipu", "gemini")
        if "/" not in v:
            raise ValueError(
...
# ==================== Failover Config ====================
# Enable automatic failover when a provider fails (rate limit, error, etc.)
failover_enabled: bool = Field(default=False, validation_alias="FAILOVER_ENABLED")
# Comma-separated list of provider types in priority order
# Example: "open_router,deepseek,groq,nvidia_nim"
failover_priority: str = Field(default="", validation_alias="FAILOVER_PRIORITY")
# Max retries per provider before moving to next
failover_max_retries: int = Field(default=2, validation_alias="FAILOVER_MAX_RETRIES")
# Enable fallback to local providers (ollama, lmstudio) when remote fails
failover_fallback_to_local: bool = Field(default=True, validation_alias="FAILOVER_FALLBACK_TO_LOCAL")
