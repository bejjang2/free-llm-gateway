...
import { dashboardApi, type ModelProvider } from '@/lib/api'
...
export default function PlaygroundPage() {
  ...
  const { data: modelsData } = useQuery<{ providers: Record<string, ModelProvider> }>({
    queryKey: ['models'],
    queryFn: () => dashboardApi.getModels(),
  })

  const providers = modelsData?.providers ?? {}
  const providerList = Object.entries(providers).filter(([_, p]) => p.has_key !== false)
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const body: any = {
        model: selectedProvider === 'auto'
          ? 'nvidia_nim/z-ai/glm4.7'
          : `${selectedProvider}/${providers[selectedProvider]?.models?.[0] ?? 'default'}`,
        max_tokens: 2048,
      ...
      if (!res.ok) {
        ...
        setMessages([...newMessages, {
          role: 'assistant',
          content: `Error: ${err.detail ?? err.error?.message ?? 'Unknown error'}`,
        }])
        return
      }
      ...
      setMessages([...newMessages, {
        ...
        meta: {
          provider: selectedProvider === 'auto' ? 'router' : selectedProvider,
          model: body.model,
          latency,
        },
      }])
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <PageHeader
        title="Playground"
        description="Chat with any provider through the unified gateway."
        actions={
          <>
            <Select value={selectedProvider} onValueChange={(v) => setSelectedProvider(v ?? 'auto')}>
            ...
            <div className="flex items-center justify-center h-full text-center">
              <div className="space-y-2 max-w-sm">
                ...
                <p className="text-sm text-muted-foreground">
                  Using <span className="text-foreground">{selectedProvider === 'auto' ? 'Auto Failover' : selectedProvider}</span>.
