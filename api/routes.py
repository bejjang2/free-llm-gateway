...
from providers.failover import get_failover_manager
...
async def create_message(
    ...
    try:
        ...
        # Initialize failover if enabled
        failover = get_failover_manager()
        if failover.enabled:
            init_failover = True  # Will be set after settings loaded
            # Parse available providers
            available = failover.get_available_providers(settings)
            logger.debug("FAILOVER: available providers: {}", available)
        else:
            available = []
        ...
        # Try request with failover logic
        attempt = 0
        max_attempts = len(available) + 1 if failover.enabled else 1
        ...
        while attempt < max_attempts:
            try:
                ...
                provider = get_provider_for_type(current_provider)
                ...
                # Rate limit - try next provider if failover enabled
                if failover.enabled:
                    failover.record_failure(current_provider, str(e))
                    next_provider = failover.get_next_provider(current_provider, available)
                    if next_provider:
                        logger.warning(
                            "FAILOVER: Rate limited on '{}', trying '{}'",
                            current_provider,
                            next_provider,
                        )
                ...
                # Other errors - try next provider if failover enabled
                if failover.enabled and attempt < max_attempts - 1:
                    failover.record_failure(current_provider, str(e))
                    next_provider = failover.get_next_provider(current_provider, available)
                    if next_provider:
                        logger.warning(
                            "FAILOVER: Error on '{}' ({}), trying '{}'",
                            current_provider,
                            type(e).__name__,
                            next_provider,
                        )
...
@router.post("/v1/messages/countTokens")
